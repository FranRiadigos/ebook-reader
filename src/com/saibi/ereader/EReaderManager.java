 ...
 * 
 * Lo usamos tanto para conectarnos, como para recuperar los archivos en una
 * tarea asincrona.
 */
public class EReaderManager {
	 ...
	 * 
	 * @param activity
	 *            Contexto de la actividad que lanza el metodo
	 */
	...
	public void getFilesByExtension(String path, String fileExtension,
		...
		DbxFilesTaskLoader task = new DbxFilesTaskLoader(path, fileExtension,
				maxFiles, callback);
		task.execute();
	}

	public class DbxFilesTaskLoader extends
			AsyncTask<Void, Integer, List<DbxFileInfo>> {
		...
		protected List<DbxFileInfo> doInBackground(Void... params) {
			...
			try {
				...
				/*
				 * Implementacion mas eficiente para obtener todos los archivos
				 * con extension .epub Dejando al servidor de dropbox hacer la
				 * busqueda en sus metadatos.
				 */
				fileList = mDropboxAPI.search(mPath, mFileExtension, mMaxFiles,
						false);
				...
				for (Entry entry : fileList) {
					if (entry.mimeType.compareTo("application/epub+zip") == 0) {
						 ...
						 */
						InputStream is = mDropboxAPI.getFileStream(entry.path,
								null);
